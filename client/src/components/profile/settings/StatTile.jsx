...
export default function StatTile({ icon: Icon, label, value, tone = 'primary' }) {
  ...
  return (
    <div
      className="rounded-2xl border-2 border-slate-900 bg-white p-2.5 sm:p-4 dark:border-white dark:bg-slate-900"
    ...
    >
      ...
      <p className="mt-1.5 font-mono text-lg font-black tracking-tight text-dark sm:mt-3 sm:text-2xl">{value}</p>
      <p className="mt-0.5 text-[9px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 sm:text-[11px]">{label}</p>
    </div>
  );
}
